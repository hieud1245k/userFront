...
public class TransferController {
	...
	public String betweenAccountsPOST(@ModelAttribute("transferFrom") String transferFrom,
		...
		transactionService.betweenAccountsTransfer(transferFrom, transferTo, Double.parseDouble(amount), primaryAccount,
				savingsAccount);
	...
	public String recipientEdit(@RequestParam("recipientName") String recipientName, Model model, Principal principal) {
		Recipient recipient = transactionService.findRecipientName(recipientName);
		List<Recipient> recipientList = transactionService.findRecipientList(principal);
		model.addAttribute("recipientList", recipientList);
		model.addAttribute("recipient", recipient);
		return "recipient";
	...
	public String toSomeoneElsePOST(@ModelAttribute("recipientName") String recipientName,
		...
		User user = userService.findByUsername(principal.getName());
		Recipient recipient = transactionService.findRecipientName(recipientName);
		transactionService.toSomeoneElseTransfer(recipient, accountType, Double.parseDouble(amount),
